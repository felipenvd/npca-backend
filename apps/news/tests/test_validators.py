...
from apps.news.validators import validate_news_cover
...
def image_upload(
    *,
    ...
    buffer = BytesIO()
    Image.new("RGB", (32, 24), "#00bab3").save(buffer, format=image_format)
...
def test_accepts_valid_jpeg_png_and_webp() -> None:
    validate_news_cover(image_upload())
    validate_news_cover(
...
def test_rejects_invalid_and_mismatched_files() -> None:
    invalid = SimpleUploadedFile("cover.webp", b"not an image", content_type="image/webp")
    with pytest.raises(ValidationError, match="não é uma imagem válida"):
        validate_news_cover(invalid)

    with pytest.raises(ValidationError, match="extensão"):
        validate_news_cover(image_upload(filename="cover.png"))
...
def test_rejects_image_over_twenty_megapixels() -> None:
    ...
    with (
        patch("apps.news.validators.Image.open", return_value=opened_image),
